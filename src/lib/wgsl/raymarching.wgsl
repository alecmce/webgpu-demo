...
fn rayMarching(ray: Ray) -> vec4<f32> {
  var length = 0.0;
  ...
  // return vec4<f32>(uniforms.background, 1.0);
}

fn process(ray: Ray, sdf: Sdf, point: vec3<f32>) -> vec4<f32> {
  switch (ray.output_type) {
    case OUTPUT_NORMALS: {
      return vec4<f32>(abs(sdf.normal), 1.0);
    }
    default: {
      var shading = ToonShading(sdf.color, 0.8, 0.3, point, sdf.normal, uniforms.light, ray.direction);
